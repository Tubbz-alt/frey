...
while IFS= read -r -d '' freyFile; do
  ...
  pushd ${infraDir} > /dev/null
    [[ -z $(git status -s) ]] || (echo "Aborting due to dirty git index at '${infraDir}'."; git diff |cat; git status ; exit 1)
    git checkout master
    git pull
    ...
    ${__root}/node_modules/.bin/replace "/apt/(\d+.\d+.\d+)" 'apt/1.3.0' "${freyFile}"
